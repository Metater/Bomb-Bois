...
public class PlayerOld : NetworkBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            // Player interaction
            if (hit.transform.gameObject.TryGetComponent<PlayerOld>(out var player))
            {
    ...
    public void CmdPunch(uint netId)
    {
        ...
        && id.gameObject.TryGetComponent(out PlayerOld player)
        && player.IsPunchable()
    ...
    [Command]
    public void CmdPlayAudioClipOnPlayer(int index, uint netId)
    {
        if (manager.TryGetPlayerWithNetId(netId, out var player))
        {
            player.RpcPlayAudioClipOnPlayer(index, netId);
        }
    }
    [ClientRpc]
    public void RpcPlayAudioClipOnPlayer(int index, uint netId)
    {
        if (manager.TryGetPlayerWithNetId(netId, out var player))
        {
            player.phoneAudioSource.PlayOneShot(manager.phoneManager.audioClips[index]);
        }
    }
}
*/